  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/vendors/new" className={buttonPrimaryClass}>
          + Add Vendor
        </Link>
      </div>
      ...
      {vendors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {vendors.map((v) => (
            <Link key={v.id} href={`/vendors/${v.id}`} className={`${cardClass} block hover:border-emerald-400 transition-colors`}>
