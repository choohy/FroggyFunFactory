...
import { createVenueViaUI, idFromUrl, uniqueName } from "./helpers/data";
...
test.describe("Venues", () => {
  ...
  test("shows the pitch, cost, and features on the detail page", async ({ page }) => {
    ...
    await expect(
      page.getByText("Stunning waterfront views and flexible indoor/outdoor flow.")
    ).toBeVisible();
